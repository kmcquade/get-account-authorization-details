...
@click.option(
    ...
    "profile here. Defaults to the 'default' profile. Environment variables will override the use of the credentials file.",
)
@click.option(
    "--output",
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Include unattached IAM policies as well as attached policies.",
)
@click_log.simple_verbosity_option(logger)
def download_authorization_details(
    profile, output, credentials_file, include_non_default_policy_versions, include_unattached
):
    """
    Runs aws iam get-authorization-details on all accounts specified in the aws credentials file, and stores them in
    account-alias.json
    ...
    for profile in profiles:
        ...
        get_account_authorization_details(
            profile, output, include_non_default_policy_versions, include_unattached
        )


def get_account_authorization_details(
    profile, output, include_non_default_policy_versions=False, include_unattached=False
):
    """
    ...
    for page in paginator.paginate(Filter=["Role"]):
        ...
        for policy in page["Policies"]:
            ...
            # TODO: Do a lookup of the default SLR policies, which cannot be modified and could create messy results.
    ...
    for page in paginator.paginate(Filter=["AWSManagedPolicy"]):
        for policy in page["Policies"]:
            # Add customer-managed policies IF they are attached to IAM principals
            #   or if `--include-unattached` is specified
            if policy["AttachmentCount"] > 0 or include_unattached:
...
if __name__ == '__main__':
    download_authorization_details()
